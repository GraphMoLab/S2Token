...
class LLaMoTokenizer(LlamaTokenizer):
    ...
    def encode_prompt(self, prompt, g, max_length, batch_max_motif_size, no_eos=False):
        ...
        batch_max_motif_size = {MEDIA_TOKENS["graph"][0]: batch_max_motif_size}  # token lengths

        # output enc_chunk
        enc_chunk = [self.bos_token_id]
        label_chunk = [0]
        attention_mask_chunk = [1]
            ...
            if m['role'] == "system":
                ...
                for _, chunk_str in enumerate(chunk_strs):
                    if chunk_str in self.media_tokens:
                        enc_chunk += [self.media_tokens[chunk_str]] * batch_max_motif_size[chunk_str]
                        label_chunk += [0] * batch_max_motif_size[chunk_str]
                        attention_mask_chunk += [1] * g.num_substructs
                        attention_mask_chunk += [0] * (batch_max_motif_size[chunk_str] - g.num_substructs)
                    else:
                        curr_chunk = self(chunk_str, add_special_tokens=False)["input_ids"]
                        enc_chunk += curr_chunk
                        label_chunk += [0] * len(curr_chunk)
                        attention_mask_chunk += [1] * len(curr_chunk)
        ...
        attention_mask_chunk = attention_mask_chunk[: max_length + 1]
        L = len(enc_chunk)
        # print(batch_max_motif_size)
        # print(L, len(label_chunk), len(attention_mask_chunk))
        assert L == len(label_chunk) == len(attention_mask_chunk)

        input_ids = torch.as_tensor(enc_chunk, dtype=torch.long)
        loss_mask = torch.as_tensor(label_chunk, dtype=torch.long)
        attention_mask = torch.as_tensor(attention_mask_chunk, dtype=torch.long)
...
def build_llamo_tokenizer(pretrained_tokenizer_name_or_path, num_graph_tokens: int):
    ...
    tokenizer = LLaMoTokenizer.from_pretrained(pretrained_tokenizer_name_or_path,
                                               use_fast=False,
                                               add_eos_token=True
                                               )
