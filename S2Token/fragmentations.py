...
# from bbb.breaking_bridge_bonds import (
#     MotifExtractionSettings, MotifVocabularyExtractor,
#     find_motifs_from_vocabulary)
# from magnet.magnet import MolDecomposition
# from psm.mol_bpe import Tokenizer, graph_bpe_smiles

fragment2type = {"ring": 0, "path": 1, "junction": 2}
ATOM_LIST = ["C", "N", "O", "F", "P", "S", "Cl", "Br", "I", "B", "Cu", "Zn", 'Co', "Mn", 'As', 'Al', 'Ni', 'Se', 'Si',
             'H', 'He', 'Li', 'Be', 'Ne', 'Na', 'Mg', 'Ar', 'K', 'Ca', 'Sc', 'Ti', 'V', 'Cr', 'Fe', 'Ga', 'Ge', 'Kr',
             'Rb', 'Sr', 'Y', 'Zr', 'Nb', 'Mo', 'Tc', 'Ru', 'Rh', 'Pd', 'Ag', 'Cd', 'In', 'Sn', 'Sb', 'Te', 'Xe', 'Cs',
             'Ba', 'La', 'Ce', 'Pr',
...
class FragmentRepresentation(BaseTransform):
    ...
    def __call__(self, graph):
        ...
        substruct_dict = {}
        for node_id, frag_list in enumerate(graph.substructures):
            for (frag_id, _) in frag_list:
                if frag_id not in substruct_dict:
                    substruct_dict[frag_id] = []
                substruct_dict[frag_id].append(node_id)

        ptr = [0]
        nodes = []
        substructures = []
        for fid in substruct_dict.keys():
            nodes.extend(substruct_dict[fid])
            ptr.append(ptr[-1] + len(substruct_dict[fid]))
            substructures.append(fid)

        graph.substruct_id = substructures   # list
        graph.substruct_ptr = torch.tensor(ptr, dtype=torch.long)
        graph.substruct_nodes = torch.tensor(nodes, dtype=torch.long)
        graph.num_substructs = len(substruct_dict.keys())
        # ------------------------------------------------------------------------
